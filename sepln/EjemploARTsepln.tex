...
\seplnresumen{
La Extracción de Argumentos es un problema que tradicionalmente es resuelto mediante la anotación
manual de expertos en lingüística, trabajo que se caracteriza por tomar mucho tiempo y recursos. 
El presente trabajo propone y muestra los resultados de aplicar algoritmos de aprendizaje profundo 
al campo de la Extracción de Argumentos en la prensa cubana, en donde dichos estudios son escasos.
Para esto se presenta un programa capaz de: 1) crear conjuntos de datos a partir de otros idiomas para
el entrenamiento de modelos, 2) entrenar los modelos propuestos y 3) realizar
la anotación automática de las unidades de discurso argumentativas (UDA).
De los conjuntos de datos disponibles, se realiza un análisis en el que se exponen las ventajas y 
deficiencias de cada uno para la anotación de las ``Cartas a la Dirección'' del periódico cubano Granma. 
Los resultados obtenidos en la extracción UDAs alcanzaron valores de
...
\seplnabstract{
	Argument Extraction is a problem that is traditionally solved by manual annotation by linguistic 
	experts, a work that takes a lot of time and resources. This paper proposes and shows the results 
	of applying deep learning algorithms to the field of Argument Extraction in the Cuban press, where 
	such studies are scarce. For this purpose, a program capable of: 1) creating data sets from other 
	languages for model training, 2) training the proposed models and 3) performing the automatic 
	annotation of argumentative discourse units (ADUs) is presented. From the available datasets, an 
	analysis is carried out in which the advantages and deficiencies of each one for the annotation of
	the ``Letters to the Management'' of the Cuban newspaper Granma are exposed. The results obtained 
	in the ADUs extraction reached values of 0.82 in the F1 metric compared to 0.85 obtained in the 
	state of the art. In the other tasks, the results are not directly comparable with those of the 
	state of the art, the best F1 values obtained were 0.56 in ADUs classification, 0.74 in link 
	prediction and 0.39 in link classification.
}
...
\setlength\titlebox{21cm} % se aumenta el tamaño del espacio reservado para datos de título
...
delimitan y clasifican las unidades de discurso argumentativas (UDA), en el segundo se clasifica 
una tupla en la que su primer elemento constituye el texto candidato de donde parte la relación 
o fuente y el segundo, el texto candidato a recibir la 
relación u objetivo, en el tipo de relación existente entre estos elementos. Los modelos utilizan 
redes neuronales convolucionales (CNN), \textit{Long Short Term Memory} (LSTM) y \textit{Conditional Random Field} (CRF)
como elementos principales en sus arquitecturas, además se emplean los vectores GloVe para la representación
de las palabras.
...
% TODO Hablar un poco de la representacino de los datos en el articulo.
...
\subsection{Preliminares de Extracción de Argumentos} %% TODO MISSING RESUMED
...
\subsection{Proyección de corpus} %% RESUMED % TODO No hacerlo seccion
...
% A continuación se presentan dos ejemplos\footnote{Extraído del corpus de \cite{stab2017parsing}.} del 
% procedimiento descrito. En estos se observa el proceso de alineación
% de los índices de los tokens en inglés con sus respectivos tokens en español, esta relación puede ser de 
...
% TODO Hablar de: 
	% [ ] Como se modela el problema en cuestion
	% [ ] Como se repersentan las palabras y porque
	% [ ] Poner solamente el modelo que se escoge y usa al final
...
\begin{figure}[h]
	\centering
	\includesvg[width=7cm]{Graphics/Modelo_Segmenter_UDA.svg}
...
% TODO Hablar de: 
	...
	% [ ] Como se repersentan las palabras y porque
	% [ ] Poner solamente el modelo que se escoge y usa al final
	% [ ] Poner detalles de implementacion como el doble for por los pares posibles de UDAs en el documento
...
\begin{figure}[h]
	\centering
	\includesvg[width=7cm]{Graphics/Modelo_Link_Prediction1.svg}
...
español y se les realizó un aumento de datos. 
...
% TODO Posible resumen: Solamente decir que se utilizo AwesomeAlign
...
% TODO Resumir esto de seleccion de modelo, Darle introduccion
...
% TODO Resumir esto de seleccion de modelo
