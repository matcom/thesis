\chapter{Marco teórico y preliminares}\label{chapter:background}

En este capítulo se introducen conceptos de aprendizaje automático. Temas
como la representación de datos, arquitecturas, evaluación de modelos son tratados en sus secciones.
Se menciona las diferentes investigaciones realizadas en la EA a lo largo de los años, viendo
...
En problemas de clasificación son empleadas medidas que toman en cuenta la naturaleza discreta de su conjunto imágen.
...
En [\cite{persing2016end}] se presenta una variante de F1 para secuencias. Esta versi'on permite
...
si coinciden en una proporci'on mayor o igual a la mitad se considera como un verdadero positivo. La 
definición de los conjuntos de verdaderos positivos, negativos y falsos negativos es:
...
\end{equation}
donde $gl(j)$ es la etiqueta correcta para $j$, $pl(i)$ es la etiqueta inferida para $i$,
$n$ es la clase no argumentativa, $i = j$ significa que $i$ es una coincidencia para $j$.
...
a los caracteres o también en listas de palabras, donde se mediría la cantidad de palabras que 
se tienen qe cambiar. 
...
que se entrena el modelo, mientras que la de entrenamiento mejora la de validación tiende a empeorar (\ref{fig:overfit}). 
...
presentan valores pequeños y similares, consecuentes con una correcto aprendizaje y generalización (\ref{fig:good_fit}).
...
siempre por debajo de la de entrenamiento (\ref{fig:unrepresentative_data}).
...
\subsection{Traducción y alineación de oraciones}
...
tiempo de cómputo y además de una mejora al modelo en versiones pasadas de los modelos IBM, 
