...
de dimensión $n$ que representa las clasificaciones inferidas por el modelo.
...
\begin{figure}[p]
	\begin{center}
		\begin{center}
            \includesvg[scale=.7]{Graphics/Modelo_Segmenter_UDA.svg}
        \end{center}
...
Este modelo consiste en dado dos pares de UDAs, 
...
tipo de UDA objetivo respectivamente $(r \in R, s \in U, t \in U)$ donde $R$ es el conjunto de 
posibles tipos de relaciones más una clasificación que representa que no están relacionadas y $U$ 
el de los tipos de UDAs del corpus con que se entrena.

\subsection{Modelo de predicción y clasificación de enlaces}

Sea dos UDAs, $S$ y $T$, donde $S$ representa la fuente de la relación, mientras que $T$ representa
...
Dado que se realiza un aprendizaje de varias tareas, se tienen varias funciones de pérdida, 
por lo que se ajustaron los aportes de los clasificadores 
...
que no se enlacen con ellos mismos, ejemplo $a \rightarrow a$, y que su distancia argumentativa absoluta sea menor 
...
devueltas por el modelo, esta salida constituye si el par está enlazado o no directamente, para este cálculo se 
...
\begin{figure}[p]
    \begin{center}
        \includesvg[scale=.7]{Graphics/Modelo_Link_Prediction1.svg}
        \caption{Predictor de enlaces 1.}\label{fig:link_predictor_model1}
    \end{center}
\end{figure}
\begin{figure}[p]
    \begin{center}
        \includesvg[scale=.7]{Graphics/Modelo_Link_Prediction2.svg}
    \end{center}
